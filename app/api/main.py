...
# ---------------------------------------------------------
# RAG aggregate production metrics
# ---------------------------------------------------------

@app.get("/metrics/rag")
def rag_metrics():
    """
    Return aggregate operational metrics for the
    EquityAI RAG service.

    The endpoint exposes operational telemetry only.
    ...
    - API credentials,
    - embeddings,
    - or other sensitive application data.
    """
    ...
    try:
        ...
        logger.info(
            "Retrieved aggregate RAG metrics"
        )
...
def ask(
    ...
    The HTTP request ID generated by the middleware is
    passed into the RAG service so one correlation ID
    traces the complete request.
    """
